...
dash_app.layout = html.Div([
    html.H1("SDTM Quality Checks", style={"text-align": "center", "color": "#2E8B57"}),
    ...
    html.Div([
        ...
        html.Div([
            ...
            dcc.Dropdown(id="study-dropdown", placeholder="Choose a study...")
        ], style={"width": "30%", "display": "inline-block"}),

        html.Div([
            html.Label("Select Analysis Folder"),
            dcc.Dropdown(id="analysis-folder-dropdown", placeholder="Choose an analysis folder...")
        ], style={"width": "30%", "display": "inline-block"})
    ], style={"margin-bottom": "20px"}),

    html.Button("Run Checks", id="run-checks-button", style={"background-color": "#2E8B57", "color": "white"}),
...
def update_study_dropdown(selected_project):
    if not selected_project:
        raise PreventUpdate
    return [{"label": study, "value": study} for study in get_studies(selected_project)]
...
def update_analysis_folder_dropdown(selected_project, selected_study):
    if not selected_project or not selected_study:
        raise PreventUpdate
    return [{"label": folder, "value": folder} for folder in get_analysis_folders(selected_project, selected_study)]
...
# PyQt MainWindow class
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PyQt and Dash Integration")
        self.setGeometry(100, 100, 1280, 720)

        # Set up layout and QWebEngineView
        ...
        self.dash_thread = Thread(target=self.run_dash_app)
        self.dash_thread.daemon = True
        self.dash_thread.start()

        # Load the Dash app into the browser widget
