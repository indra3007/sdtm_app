...
def open_folder():
    ...
    return jsonify({"success": False, "message": "Invalid folder path or folder does not exist."})
...
def home_section():
    return html.Div([
        ...
        html.Div([
            html.Button(
                [html.I(className="bi bi-info-circle"), " Info"],  # Icon and text
            ...
            ], id="info-popup", className="info-popup", style={"display": "none"})  # Initially hidden
        ], className="info-container")  # Container for the info button and popup
    ], className="home-container")  
@app.callback(
    Output("info-popup", "style"),  # Toggle the popup's visibility
    [Input("info-button", "n_clicks"), Input("ok-button", "n_clicks")],  # Triggered by Info or OK button
...
def protocol_page():
    protocol_df = summary_df.groupby("Protocol").agg({
        "Data_Path": "first",
        "Date": "first"
    }).reset_index()
    ...
    return html.Div([
        ...
        home_section(),
        html.Div([
            html.I(className="bi bi-folder-fill folder-icon", style={"marginLeft": "25px", "marginRight": "10px","color": "#007bff","fontSize": "1.5rem","marginBottom": "10px"}), 
            html.H1(
                "Protocols",
                style={
        ...
        ], style={"display": "flex", "alignItems": "center", "marginTop": "20px","marginBottom": "10px","marginRight": "25px"}),  
        html.Div(
            dag.AgGrid(
                id="protocol-grid",
                ...
                columnDefs=[
                    ...
                    {"headerName": "Study Path", "field": "Data_Path", "width": 1200},
...
def project_page(selected_protocol):
    ...
    project_df = filtered_df.groupby("Project").agg({
        "Data_Path": "first"
    }).reset_index()
    ...
    return html.Div([
        ...
        home_section(),
        html.Div([
            html.I(className="bi bi-folder-fill", style={"marginLeft": "25px", "marginRight": "10px","color": "#007bff", "fontSize": "1.5rem","marginBottom": "10px"}), 
            html.H1(
...
def analysis_task_page(selected_protocol, selected_project):
    ...
    task_df = filtered_df.groupby("Analysis_Task").agg({
        "Data_Path": "first"
    }).reset_index()
    ...
    return html.Div([
        ...
        home_section(),
        html.Div([
            html.I(className="bi bi-list-task", style={"marginLeft": "25px", "marginRight": "10px", "color": "#007bff", "fontSize": "1.5rem", "marginBottom": "10px"}), 
            html.H1(
...
def analysis_version_page(selected_protocol, selected_project, selected_task):
    ...
    filtered_df = summary_df[
        (summary_df["Protocol"].str.strip().str.lower() == selected_protocol.strip().lower()) &
        (summary_df["Project"].str.strip().str.lower() == selected_project.strip().lower()) &
        (summary_df["Analysis_Task"].str.strip().str.lower() == selected_task.strip().lower())
    ]
    ...
    filtered_df["SDTM_Spec"] = filtered_df["SDTM_Path"].apply(lambda path: f"{base_url}{path}")
    ...
    version_df = filtered_df.groupby(["Protocol", "Project", "Analysis_Version", "SDTM_Spec","Max_Modified_Date"]).agg({
        "Data_Path": "first"
    }).reset_index()
    writer = pd.ExcelWriter("testing_analysis_version.xlsx", engine="xlsxwriter")
    ...
    return html.Div([
        ...
        home_section(),
        html.Div([
            html.I(className="bi bi-bar-chart-fill", style={"marginLeft": "25px", "marginRight": "10px", "color": "#007bff", "fontSize": "1.5rem", "marginBottom": "10px"}), 
            html.H1(
        ...
        html.Div(
            dag.AgGrid(
                ...
                columnDefs=[
                    ...
                    {"headerName": "SDTM Run Date", "field": "Max_Modified_Date", "width": 200},
                ],
                dashGridOptions={
                    "pagination": True,
...
def display_table_page(selected_prot, selected_project, selected_task, selected_version):
    ...
    layout = [
        ...
        html.Div([
            html.I(className="bi bi-table", style={"marginLeft": "25px", "marginRight": "10px", "color": "#007bff", "fontSize": "1.5rem", "marginBottom": "10px"}), 
            html.H1(
    ...
    if selected_task.strip().lower() == "csdtm_dev":
        layout.append(
            html.Div([
                ...
                dcc.Store(id="quality-checks-path", data=quality_checks_path),  # Store the folder path
                html.Div(id="quality-checks-message", style={"marginTop": "10px", "color": "red"}),  # Message display
...
def display_pinnacle_21_page(selected_prot, selected_project, selected_task, selected_version):
    try:
    ...
    if pathname == "/":
        ...
        if len(parts) == 2:
            ...
            if not protocol_row.empty:
                ...
                return html.Div(f"Invalid URL: Protocol could not be determined for Project {selected_project}.")
        ...
        if len(parts) == 2:
            ...
            task_row = summary_df[
                ...
                (summary_df["Protocol"] == selected_protocol) & 
                (summary_df["Project"] == selected_project) # Filter by global protocol
            ]
            ...
            if not task_row.empty:
                ...
                print(f"Resolved as Analysis Task: {selected_task}, Protocol: {selected_protocol}, Project: {selected_project}")
                return analysis_version_page(selected_protocol, selected_project, selected_task)

            return html.Div(f"Invalid URL: {value} is not valid for the current Protocol.")
        ...
        # Handle 4-part URLs (e.g., /analysis-version/<protocol>/<project>/<task>)
            ...
            print(f"Resolved 4-part URL - Protocol: {selected_protocol}, Project: {selected_project}, Task: {selected_task}")
        else:
        ...
        if len(parts) == 3:
            ...
            protocol_row = summary_df[
                (summary_df["Protocol"] == selected_protocol) &  # Filter by global protocol
                (summary_df["Project"] == selected_project) &
            ...
            print(f"Filtered DataFrame for Protocol {selected_protocol} and Project {selected_project} and Task {selected_task} and Version {selected_version} ")
            #print(protocol_row)

            if not protocol_row.empty:
                selected_project = protocol_row["Project"].iloc[0]
            else:
                return html.Div(f"Invalid URL: Protocol or Project could not be determined for Task {selected_task} and Version {selected_version}.")
        ...
        print(f"Resolved Values - Protocol: {selected_protocol}, Project: {selected_project}, Task: {selected_task}, Version: {selected_version}")  # Debugging
        return display_table_page(selected_protocol, selected_project, selected_task, selected_version)
...
def update_back_link(pathname):
    ...
    if pathname.startswith("/project/"):
        ...
        if len(parts) == 3:
            ...
            print(f"Filtered DataFrame for Task {selected_task} and Version {selected_version}:")
            #print(protocol_row)
...
def handle_quality_checks_button(n_clicks, folder_path):
    ...
    if folder_path:
        try:
            ...
            response = requests.post("http://127.0.0.1:8050/open-folder", json={"folder_path": folder_path})
