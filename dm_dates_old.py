# -*- coding: utf-8 -*-
"""
Created on Wed Aug 21 13:07:25 2024
...
def process_datasets(df_xl, sas_directory, aggregation='max', remove_time=False, all_dates=False, remove_partial=True):
                ...
                if not var_name.endswith('_RAW'):
                    ...
                    if os.path.exists(dataset_file):
                        ...
                        if raw_var_name in df.columns:
                            ...
                            #df_xl.loc[idx, 'Input Variables'] = df_xl.loc[idx, 'Input Variables'].replace(var, f"{parts[0]}.{parts[1]}.{raw_var_name.lower()}")
                            #comments.append(f"Update {parts[2]} to {raw_var_name} for row {idx + 1}")
    ...
    for dataset_name in set(date_vars_dict) | set(time_vars_dict):
        ...
        for date_var in date_vars:
            ...
            if ttm_var in filtered_df.columns:
                filtered_df[date_var] = filtered_df.apply(
                    lambda row: combine_date_time(row[date_var], row[ttm_var]) if pd.notna(row[ttm_var]) else row[date_var],
                    axis=1
                )
        ...
        # Unpivot the date columns
        df_unpivoted = filtered_df.melt(id_vars=['X_SUBJID', 'dataset', 'DATAPAGENAME'],
    ...
    final_df['Dates'] = final_df['Dates'].apply(safe_convert)
    ...
    max_dates_with_datapagename = pd.merge(max_dates_df, final_df[['X_SUBJID', final_col_name, 'DATAPAGENAME','Var_Name']], on=['X_SUBJID', final_col_name], how='left')
